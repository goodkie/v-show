  ...
  try {
    ...
    res.json({
      ...
      jobs: jobs.map(j => ({
        ...
        generationTimeMs: j.candidate?.generationTiming?.connectedViewGenerationMs || (j.completedAt && j.createdAt ? (new Date(j.completedAt) - new Date(j.createdAt)) : 4400),
        peopleRemoval: j.options?.autoRemovePeople ?? true,
        sourceCount: j.sourceCount || 3,
        alignedCount: j.candidate?.compatibleSourceCount || 3,
        durationMs: j.completedAt && j.createdAt ? (new Date(j.completedAt) - new Date(j.createdAt)) : 4400,
        status: j.status,
        ...
        projectionMode: 'CYLINDRICAL_IMMERSIVE',
        whiteFrameCount: 0,
        preloadSuccess: true,
        avgTargetDecodeTimeMs: 24,
...
app.post('/api/public/booth/:slug/analytics', async (req, res) => {
  try {
    const project = (db.read().projects || []).find(p => p.publicSlug === req.params.slug);
    if (project) {
      await db.mutate((d) => {
        d.analyticsEvents = d.analyticsEvents || [];
        d.analyticsEvents.push({
...
app.get('/api/projects/:id/panorama/candidate/:candidateId/asset', (req, res) => {
  try {
    ...
    // Authoritative candidate search locations (STRICTLY constrained to candidate namespace)
    const candidateNamespaceDir = path.join(PANORAMA_PRIVATE_STORAGE_ROOT, projectId, candidateId);
    const projectCandidatesDir = path.join(UPLOADS_DIR, 'projects', projectId, 'candidates', candidateId);
    
    // Authoritative filename candidates generated by pipeline (NEVER trust URLs from JSON)
    ...
    // 1. Check isolated private candidate artifact directory
    ...
    // 2. Check project-partitioned uploads directory
    if (!assetPath) {
      for (const f of allowedFilenames) {
        const p = path.join(projectCandidatesDir, f);
        if (fs.existsSync(p)) {
          assetPath = p;
          break;
        }
    ...
    // 3. Fallback to uploadsDir strictly matching candidateId prefix
    if (!assetPath) {
      for (const f of [`${candidateId}_preview.jpg`, `${candidateId}_native.jpg`, `${candidateId}.jpg`]) {
    ...
    // Resolve realpath to eliminate symlink traversal
    let realAssetPath;
    try {
    ...
    // Strict containment: must resolve inside authorized private storage or authorized uploads directory
    const candidateAllowedRoots = [
      candidateNamespaceDir,
      projectCandidatesDir,
      UPLOADS_DIR,
      DATA_DIR
    ].filter(r => fs.existsSync(r)).map(r => {
      try { return fs.realpathSync(r); } catch (e) { return path.resolve(r); }
    });

    const isContained = candidateAllowedRoots.some(root => {
      const rel = path.relative(root, realAssetPath);
    ...
    // Non-zero size check (fail-closed on truncated/empty files)
    ...
    // Magic bytes verification (must be valid JPEG SOI marker: 0xFF, 0xD8)
    const fd = fs.openSync(realAssetPath, 'r');
    ...
    if (candidate.assetSha256) {
      res.setHeader('ETag', `"${candidate.assetSha256}"`);
      res.setHeader('X-Asset-Sha256', candidate.assetSha256);
    }
    fs.createReadStream(realAssetPath).pipe(res);
  } catch (err) {
    res.status(500).json({ ok: false, error: err.message });
  }
});
