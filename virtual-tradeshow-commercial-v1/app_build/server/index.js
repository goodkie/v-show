...
app.get('/api/projects/:id/panorama/candidate/:candidateId/asset', (req, res) => {
  try {
    ...
    // Strict containment: must resolve strictly inside exact candidate namespace directory (no broad UPLOADS or DATA_DIR)
    ...
    // Read full buffer to recompute and cryptographically verify byte checksum before streaming
    const fileBuffer = fs.readFileSync(realAssetPath);
    const computedSha256 = crypto.createHash('sha256').update(fileBuffer).digest('hex');

    // If candidate record contains an authoritative assetSha256, strictly enforce byte-for-byte equality
    if (candidate.assetSha256 && candidate.assetSha256 !== computedSha256) {
