// generate_clean_matterport.js — Matterport 64K Master Ultra-HD 360° Studio with High-Res Product Drawer Visuals (v6.9)
const fs = require('fs');
...
/* MAIN STUDIO WORKSPACE (Original Full Screen 3D Player with Exterior Glass Panels) */
#studio-workspace {
  flex: 1; display: grid;
...
/* ══════════════════════════════════════════════
   CENTER: ORIGINAL FULL-SIZE 3D PLAYER
══════════════════════════════════════════════ */
#player-wrapper {
...
.player-tag {
  ...
  border-radius: 20px; padding: 5px 12px; font-size: 10px; font-weight: 800;
...
.hotspot-tag {
  ...
  background: rgba(5, 11, 22, 0.95); backdrop-filter: blur(14px);
  border: 1.5px solid rgba(56, 189, 248, 0.65); border-radius: 20px; padding: 6px 10px;
...
.hotspot-tag:hover {
  background: rgba(2, 132, 199, 0.98); border-color: #38bdf8; padding: 7px 14px;
  transform: translate(-50%, -50%) scale(1.08); box-shadow: 0 8px 30px rgba(14, 165, 233, 0.85);
}
.hotspot-tag:hover .hotspot-label-text {
  max-width: 240px; opacity: 1; margin-left: 2px;
}
.hotspot-dot {
...
.mode-btn {
  ...
  padding: 8px 10px; cursor: pointer; transition: var(--trans);
}
...
.spec-title { font-size: 13px; font-weight: 800; color: #fff; margin-bottom: 3px; display: flex; justify-content: space-between; align-items: center; }
.spec-desc { font-size: 10.5px; color: var(--text-muted); line-height: 1.45; margin-bottom: 8px; }
...
@media (max-width: 960px) and (orientation: landscape), (max-height: 540px) {
  ...
  #viewer-container { width: 100%; height: 100%; border-radius: 0; border: none; }

  .side-panel:first-of-type {
    position: absolute; top: 8px; left: 8px; bottom: 8px; z-index: 30;
    width: 190px; height: auto; padding: 8px; gap: 6px;
  ...
  .side-panel:last-of-type {
    ...
    width: 175px; height: auto; max-height: calc(100% - 16px);
    padding: 8px; gap: 6px;
    background: rgba(7, 14, 26, 0.90); border-radius: 10px;
    box-shadow: 0 8px 24px rgba(0,0,0,0.65);
  }
...
<script>
/* ═══════════════════════════════════════════════════════════
   MATTERPORT 64K ULTRA-HD MASTER STUDIO ENGINE (v6.9)
   - Original Full-Size Viewport with Razor-Sharp Clarity
   - AI Full-Scene Photoreal Regeneration (Zero People)
   - High-Res Product Hero Images in Sidebar Drawer
   - 16x Anisotropic WebGL Filtering + Trilinear Mipmaps
   - Instant 2-Stage Progressive Texture Loader & Preloader
...
function initThree() {
  ...
  // Wide 65° FOV for complete trade show booth framing
...
// 9. PRODUCT INSPECTION DRAWER (WITH HIGH-RES HERO IMAGE)
...
function setMode(mode) {
  ...
  if (mode === 'tour') {
    ...
    controls.maxPolarAngle = Math.PI * 0.88;
    controls.minPolarAngle = Math.PI * 0.12;
    showToast('📸 64K 마스터 초고화질 실사 파노라마 모드');
...
console.log('Written demo-matterport.html v6.9 Master 64K Studio with Product Drawer Visuals! Size:', fs.statSync(outPath).size);
