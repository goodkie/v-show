...
<title>DN'a ROBOTIC | Matterport 3D Ultra-HD Digital Twin Virtual Tour</title>
...
<div id="topbar">
  <div class="brand-group">
    ...
    <div class="brand-badge"><div class="pulse-dot"></div> MATTERPORT 3D ULTRA-HD</div>
  </div>
...
<div id="node-panel">
  ...
  <button class="node-btn active" id="nb-0" onclick="switchNode(0)"><span class="node-icon">📷</span> 01. 부스 메인 조망 (Main View)</button>
  <button class="node-btn" id="nb-1" onclick="switchNode(1)"><span class="node-icon">🤖</span> 02. 전면 코봇 7대 라인업 (CoBots)</button>
  <button class="node-btn" id="nb-2" onclick="switchNode(2)"><span class="node-icon">🚗</span> 03. AMR 자율주행 물류 존 (AMR AGV)</button>
  <button class="node-btn" id="nb-3" onclick="switchNode(3)"><span class="node-icon">🖥</span> 04. AI 비전 미디어 월 (LED Wall)</button>
</div>
...
<div id="booth-radar">
  <div class="radar-label">
    ...
    <span id="radar-loc-txt">01. MAIN VIEW</span>
  </div>
...
// 7. HIGH-RESOLUTION TEXTURE LOADER
...
function toggleAutoTour() {
  ...
  if (autoTourActive) {
    ...
    showToast('▶ 4.8초 간격 Matterport 자동 가이드 투어 시작');
...
// 12. MATTERTAG SCREEN PROJECTION
