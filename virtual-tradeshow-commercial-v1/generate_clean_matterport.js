// generate_clean_matterport.js — Matterport 16K/8K Ultra-HD 360° Studio Player (v6.2)
const fs = require('fs');
...
<script>
/* ═══════════════════════════════════════════════════════════
   MATTERPORT 16K ULTRA-HD 360° STUDIO ENGINE (v6.2)
...
// 1. SPATIAL NODES (16K & 8K Ultra-HD Equirectangular Photoreal Assets)
const SPATIAL_NODES = [
  {
    id: 0,
...
// 6. HIGH-RESOLUTION TEXTURE LOADER WITH HARDWARE ADAPTATION
...
function switchNode(idx) {
  ...
  // Use hardware-supported optimal texture (16K if GPU maxTextureSize >= 16384, else 8K)
  const maxTex = renderer ? renderer.capabilities.maxTextureSize : 8192;
  const targetUrl = (maxTex >= 16384 && node.image16k) ? node.image16k : (node.image8k || node.image16k);
  ...
  new TWEEN.Tween(photoMaterial)
    ...
    .onComplete(() => {
      loadNodeTexture(targetUrl, (tex) => {
        ...
        new TWEEN.Tween(photoMaterial).to({ opacity: 1.0 }, 300).start();
      });
    })
    .start();
  ...
  showToast('📍 ' + node.name + ' 초고화질 공간으로 이동');
}
...
console.log('Written demo-matterport.html v6.2 Studio! Size:', fs.statSync(outPath).size);
