// generate_clean_matterport.js — Matterport 64K Master Ultra-HD 360° Studio (v7.0)
// Features: 2/3 Height 3D Player, Bottom Product Cards Bar, Permanent Thumbnail Hotspot Capsules, Unified Right Sidebar (No Viewport Controls)
const fs = require('fs');
...
<title>DN'a ROBOTIC | Matterport 64K Ultra-HD 360° Studio Digital Twin</title>
...
/* MAIN STUDIO WORKSPACE (Center Player + Right Unified Sidebar) */
#studio-workspace {
  flex: 1; display: grid;
...
/* ══════════════════════════════════════════════
   CENTER WORKSPACE: 2/3 SIZE 3D PLAYER + BOTTOM PRODUCT SHORTCUT CARDS
══════════════════════════════════════════════ */
#player-workspace {
...
/* ══════════════════════════════════════════════
   BOTTOM PRODUCT SHORTCUT CARDS TRAY
══════════════════════════════════════════════ */
#product-cards-tray {
...
/* ══════════════════════════════════════════════
   3D FLOATING PRODUCT HOTSPOTS (PERMANENT TITLE & THUMBNAIL)
══════════════════════════════════════════════ */
...
/* ══════════════════════════════════════════════
   RIGHT UNIFIED SIDEBAR (Vantage Points + Radar + Equipment Specs)
══════════════════════════════════════════════ */
.unified-sidebar {
...
#radar-canvas { width: 100%; height: 75px; display: block; border-radius: 6px; }

/* 3. Focus Equipment Specs Box */
.spec-panel-box {
...
.spec-title { font-size: 13px; font-weight: 800; color: #fff; display: flex; justify-content: space-between; align-items: center; }
.spec-desc { font-size: 10.5px; color: var(--text-muted); line-height: 1.45; }
...
/* ══════════════════════════════════════════════
   PRODUCT INSPECTION DRAWER & HIGH-RES VISUALS
══════════════════════════════════════════════ */
#drawer-scrim {
...
.drawer {
  ...
  width: 450px; max-width: 92vw; background: rgba(7, 14, 26, 0.98);
...
/* HIGH-RES PRODUCT HERO IMAGE BOX */
.drawer-img-box {
  width: 100%; border-radius: 12px; overflow: hidden;
  border: 1px solid var(--panel-border); margin: 6px 0 14px;
...
.drawer-img-badge {
  ...
  border-radius: 20px; padding: 3px 9px; font-size: 8.5px; font-weight: 800;
  color: var(--cyan); backdrop-filter: blur(8px); font-family: var(--mono);
}
...
@media (max-width: 960px) and (orientation: portrait) {
  ...
  #player-workspace { width: 100%; flex: 1; }
  ...
  .prod-card-title { font-size: 10.5px; }
...
@media (max-width: 960px) and (orientation: landscape), (max-height: 540px) {
  #portrait-tip-bar { display: none !important; }
  #mobile-portrait-bar { display: none !important; }
  
  #topbar { height: 38px; padding: 0 10px; background: rgba(5, 11, 22, 0.92); }
  .brand-logo { font-size: 14.5px; }
  .brand-badge { display: none; }
  .btn-ui { padding: 3px 7px; font-size: 10px; }

  #studio-workspace {
    grid-template-columns: 1fr 240px; padding: 6px; gap: 8px; height: calc(100vh - 72px);
  }
  #product-cards-tray { height: 75px; gap: 6px; }
  .prod-card-thumb { width: 45px; }
  .prod-card-title { font-size: 9.5px; }
  .prod-card-spec { display: none; }

  .unified-sidebar { padding: 8px; gap: 6px; }
  .panel-sub { display: none; }
  .node-btn { padding: 5px 6px; font-size: 9.5px; }
  .radar-box { padding: 5px; }
  #radar-canvas { height: 50px; }
  .spec-panel-box { padding: 6px; }

  #studio-footer { height: 34px; padding: 0 10px; font-size: 9.5px; background: rgba(5, 11, 22, 0.92); }
  .foot-info span:nth-child(1), .foot-info span:nth-child(2) { display: none; }
  .foot-actions .btn-ui { padding: 2px 7px; font-size: 9.5px; }

  .drawer { width: 330px; }
  .drawer-img-box img { height: 150px; }
...
<div id="app-layout">
  ...
  <header id="topbar">
    <div class="brand-group">
      ...
      <div class="brand-badge"><div class="pulse-dot"></div> 64K ULTRA-HD MASTER STUDIO</div>
    </div>
    <div class="top-actions">
      <a href="/demo.html" class="btn-ui">← 3D 쇼룸</a>
      <a href="/demo-splat.html" class="btn-ui">△ 3DGS 뷰어</a>
      <button class="btn-ui download" onclick="openDownloadModal()">📥 64K 다운로드</button>
  ...
  <div id="portrait-tip-bar" onclick="this.style.display='none'">
    📱 스마트폰을 <strong>[가로 모드]</strong>로 회전하시면 더욱 넓은 64K 360° 스튜디오 투어를 즐기실 수 있습니다!
    <span>(탭하여 알림 닫기)</span>
  </div>

  <!-- MAIN STUDIO WORKSPACE (Center Player + Right Unified Sidebar) -->
  <main id="studio-workspace">
    <!-- CENTER: 2/3 SIZE 3D PLAYER + PRODUCT SHORTCUT CARDS -->
    <section id="player-workspace">
      <div id="viewer-container">
        <canvas id="three-canvas"></canvas>
        <!-- 3D Hotspot Tags Overlay -->
        <div id="hotspot-layer"></div>
        <div class="player-tag">🌐 64K Master Ultra-HD 실사 투어</div>
        <div class="res-pill" id="res-indicator">65536 × 32768 ULTRA-HD</div>
      </div>
      ...
      <div id="product-cards-tray">
        <div class="prod-quick-card active" id="pcard-0" onclick="focusProduct(0)">
          ...
          <div class="prod-card-body">
            ...
            <div class="prod-card-spec">16.0kg Payload · 6축 정밀</div>
            <div class="prod-card-btn">상세보기 <span>→</span></div>
          </div>
        </div>
        ...
        <div class="prod-quick-card" id="pcard-1" onclick="focusProduct(1)">
          ...
          <div class="prod-card-body">
            <div class="prod-card-cat">AMR ROBOT</div>
            <div class="prod-card-title">Vector AMR 600</div>
            <div class="prod-card-spec">600kg 적재 · 3D LiDAR SLAM</div>
            <div class="prod-card-btn">상세보기 <span>→</span></div>
          </div>
        </div>
        ...
        <div class="prod-quick-card" id="pcard-2" onclick="focusProduct(2)">
          <div class="prod-card-thumb">
            <img src="/assets/demo/dna-showcase/products/vision_led_wall.jpg" alt="Vision Telemetry LED">
          </div>
          <div class="prod-card-body">
            <div class="prod-card-cat">SMART ENGINE</div>
            <div class="prod-card-title">Vision LED 월</div>
            <div class="prod-card-spec">8K Micro-LED · 실시간 관제</div>
            <div class="prod-card-btn">상세보기 <span>→</span></div>
          </div>
        </div>
        ...
        <div class="prod-quick-card" id="pcard-3" onclick="focusProduct(3)">
          <div class="prod-card-thumb">
            <img src="/assets/demo/dna-showcase/products/vip_glass_lounge.jpg" alt="VIP Glass Lounge">
          </div>
          <div class="prod-card-body">
            <div class="prod-card-cat">ARCHITECTURE</div>
            <div class="prod-card-title">VIP Glass Hub</div>
            <div class="prod-card-spec">2층 복층 통유리 · 45㎡ 라운지</div>
            <div class="prod-card-btn">상세보기 <span>→</span></div>
          </div>
        </div>
      </div>
    ...
    <aside class="unified-sidebar">
      ...
      <div class="radar-box">
        ...
        <canvas id="radar-canvas" width="306" height="75"></canvas>
      </div>

      <!-- Live Target Specs Box -->
      <div class="spec-panel-box">
        <div class="panel-head" style="margin-bottom:0;">FOCUS EQUIPMENT</div>
  ...
  <footer id="studio-footer">
    <div class="foot-info">
      <span>엔진: <strong>Three.js WebGL 64K Master</strong></span>
  ...
  <aside class="drawer" id="product-drawer">
    ...
    <div class="drawer-body">
      ...
      <div class="drawer-img-box" id="drw-img-box">
        ...
        <div class="drawer-img-badge">STUDIO PRODUCT HERO 4K</div>
      </div>
...
<div class="app-modal" id="download-modal" onclick="if(event.target===this)closeDownloadModal()">
  <div class="modal-box">
    ...
    <div style="font-size:10px;font-weight:800;color:var(--cyan);letter-spacing:1px;text-transform:uppercase;">64K MASTER ULTRA-HD ORIGINAL ASSETS</div>
    <h2 style="font-size:17px;font-weight:800;color:#fff;margin:6px 0 6px;">📸 64K 마스터 초고화질 원본 다운로드</h2>
    <p style="font-size:11px;color:var(--text-muted);line-height:1.5;margin-bottom:14px;">
      스튜디오 뷰어에 사용된 64K(65536×32768) 마스터 실사 원본 이미지입니다.
    </p>

    <div class="dl-card">
      <div class="dl-info">
        <div class="dl-title">01. 부스 메인 중앙 전경 (Middle Master)</div>
        <div class="dl-meta">Ultra-HD Equirectangular JPG (64K Master) · 3.7 MB</div>
      </div>
      <a href="/assets/demo/dna-showcase/pano360/node0_360_panorama_8k.jpg" download="DN_a_Booth_Main_64K.jpg" class="dl-btn">⬇ 다운로드</a>
    </div>

    <div class="dl-card">
      <div class="dl-info">
        <div class="dl-title">02. 코봇 워크스테이션 (Left Master)</div>
        <div class="dl-meta">Ultra-HD Equirectangular JPG (64K Master) · 3.5 MB</div>
      </div>
      <a href="/assets/demo/dna-showcase/pano360/node1_360_panorama_8k.jpg" download="DN_a_Cobot_Station_64K.jpg" class="dl-btn">⬇ 다운로드</a>
    </div>

    <div class="dl-card">
      <div class="dl-info">
        <div class="dl-title">03. AMR & 미디어 파사드 (Right Master)</div>
        <div class="dl-meta">Ultra-HD Equirectangular JPG (64K Master) · 4.0 MB</div>
      </div>
      <a href="/assets/demo/dna-showcase/pano360/node2_360_panorama_8k.jpg" download="DN_a_AMR_Facade_64K.jpg" class="dl-btn">⬇ 다운로드</a>
    </div>
...
<script>
/* ═══════════════════════════════════════════════════════════
   MATTERPORT 64K ULTRA-HD MASTER STUDIO ENGINE (v7.0)
   - 2/3 Height 3D Player + Bottom Product Shortcut Cards Bar
   - Permanent 3D Floating Capsules with Title & Product Thumbnail
   - Unified Right Sidebar (Vantage Points, Minimap, Target Specs)
   - High-Res Product Hero Images & Progressive Texture Preloader
═══════════════════════════════════════════════════════════ */

// 1. SPATIAL NODES (64K Master Panoramas - No People)
const SPATIAL_NODES = [
  {
    id: 0,
...
// 2. PRODUCTS & 3D FLOATING HOTSPOTS WITH STUDIO HERO IMAGES
const PRODUCTS_DATA = [
  {
    id: 'PROD-01-COBOT',
    name: 'Apex Cobot X16',
    ...
    worldPos: new THREE.Vector3(0, -65, -380),
    nodeIdx: 0,
  ...
  },
  {
    ...
    worldPos: new THREE.Vector3(-220, -130, -290),
    nodeIdx: 2,
  ...
  },
  {
    ...
    image: '/assets/demo/dna-showcase/products/vision_led_wall.jpg',
    worldPos: new THREE.Vector3(0, 65, -390),
    nodeIdx: 0,
  ...
  },
  {
    ...
    image: '/assets/demo/dna-showcase/products/vip_glass_lounge.jpg',
...
function initThree() {
  ...
  if (resPill) {
    resPill.textContent = maxTex >= 16384 ? '65536 × 32768 ULTRA-HD' : '16384 × 8192 UHD (MASTER)';
  }
  ...
  // Background preload all panoramas
...
function switchNode(idx) {
  ...
  drawRadar();
  updateFocusSpec(idx === 2 ? 1 : idx === 1 ? 3 : 0);
  showToast('📍 ' + node.name + ' 공간으로 이동');
}

// 8. FOCUS PRODUCT (From bottom cards or hotspots)
function focusProduct(idx) {
  ...
  // If product is in a different node, switch node first
  if (p.nodeIdx !== undefined && p.nodeIdx !== currentNodeIdx) {
    switchNode(p.nodeIdx);
  }
...
// 10. PRODUCT INSPECTION DRAWER (WITH HIGH-RES HERO IMAGE)
function openProductDrawer(idx) {
  ...
  const imgBox = document.getElementById('drw-img-box');
  if (p.image) {
    imgEl.src = p.image;
...
function downloadAllPhotos() {
  SPATIAL_NODES.forEach((n, idx) => {
    setTimeout(() => {
      ...
      a.href = n.image8k;
      a.download = 'DN_a_Master_View_' + idx + '_64K.jpg';
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
    }, idx * 600);
  });
  showToast('📥 64K 마스터 초고화질 원본 사진 다운로드가 시작되었습니다.');
}

function openRFQ() { window.open('mailto:sales@dna-robotic.com?subject=RFQ%20from%20Matterport%2064K%20Studio'); }
...
console.log('Written demo-matterport.html v7.0 with 2/3 Player, Bottom Cards Bar & Unified Right Sidebar! Size:', fs.statSync(outPath).size);
