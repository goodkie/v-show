// generate_clean_splat.js — 3D Gaussian Splatting (3DGS) Radiance Field Viewer (v8.0 Master Real 3D)
// 1. Pure Real-Time 3D Gaussian Radiance Field Engine (NO video player / NO video buttons)
// 2. High-Density Radiance Point Cloud & Volumetric Gaussian Kernels
// 3. 4 Precise Robot Exhibition Stations (Apex Cobot X16, Vector AMR 600, Titan Delta D12, Hyperion SCARA S8)
// 4. Enlarged 3D Floating Product Capsules (48px Thumbnail + Glowing Dot + Dual-Line Category/Title)
// 5. Mobile Landscape Mode Lock Overlay & Left Vertical Product Cards Tray
// 6. Unified Right Sidebar (Booth Radar + Focused Equipment Specs + 1:1 RFQ)
// 7. Product Inspection Drawer with 4K Photo & 360° 3D Mini Turntable Player
...
<meta name="description" content="Enterprise interactive 3D Gaussian Splatting (3DGS) radiance field digital twin featuring real-time 3D navigation, B2B product inspection, and RFQ intake.">
...
/* CENTER: 2/3 VIEWPORT + PRODUCT TRAY */
#player-workspace {
...
/* ══════════════════════════════════════════════
   PRODUCT SHORTCUT CARDS TRAY (4 ROBOTS)
   - Desktop: Bottom Horizontal Tray
   - Mobile Landscape: Left Vertical Column
══════════════════════════════════════════════ */
#product-cards-tray {
...
<div id="app-layout">
  ...
  <main id="studio-workspace">
    <!-- CENTER: 2/3 VIEWPORT + LEFT/BOTTOM 4-ROBOT SHORTCUT TRAY -->
    <section id="player-workspace">
      <div id="viewer-container">
        <canvas id="three-canvas"></canvas>
        <div id="hotspot-layer"></div>
        
        <div class="player-tag">△ 3DGS Real-Time Gaussian Radiance Engine</div>
        <div class="res-pill">3DGS REAL-TIME PBR</div>
      </div>
  ...
  <footer id="studio-footer">
    <div class="foot-info">
      <span>Engine: <strong>Three.js 3DGS Radiance Field Engine</strong></span>
      <span>Scene: <strong>demo_booth.ksplat (Volumetric Point Radiance)</strong></span>
...
<script>
/* ═══════════════════════════════════════════════════════════
   3DGS GAUSSIAN RADIANCE VIEWER ENGINE (v8.0 Master Real 3D)
═══════════════════════════════════════════════════════════ */
...
const PRODUCTS_DATA = [
  {
    ...
    pos3d: new THREE.Vector3(0, 0, -1.8),
  ...
  },
  {
    ...
    pos3d: new THREE.Vector3(-4.8, 0, 1.6),
  ...
  },
  {
    ...
    pos3d: new THREE.Vector3(-3.4, 0, -4.8),
...
let splatParticles = null, splatCloud = null;
...
function init3DGS() {
  ...
  scene.fog = new THREE.Fog(0x040813, 20, 60);
...
function buildBoothArchitecture() {
  ...
  const floorMat = new THREE.MeshStandardMaterial({ color: 0x070e1c, roughness: 0.22, metalness: 0.6 });
...
// 6. ENLARGED 3D FLOATING HOTSPOTS (48px Thumbnail + Dual-Line Category & Title)
...
// 8. PRODUCT INSPECTION DRAWER & 360° 3D MINI PLAYER
...
function animate(time) {
  ...
  TWEEN.update(time);
  controls.update();
  if (splatCloud) splatCloud.rotation.y += 0.0004;
  updateHotspots();
  drawRadar();
...
console.log('Written demo-splat.html v8.0 Pure 3DGS Radiance Engine! Size:', fs.statSync(outPath).size);
