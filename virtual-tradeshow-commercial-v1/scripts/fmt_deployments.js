...
const query = `query { deployments(input: { projectId: "${projectId}", serviceId: "${serviceId}", environmentId: "${environmentId}" }, first: 5) { edges { node { id status createdAt } } } }`;
...
const req = https.request('https://backboard.railway.com/graphql/v2', {
  ...
  res.on('end', () => {
    try {
      ...
      console.log('Deployments List:', JSON.stringify(data, null, 2));
