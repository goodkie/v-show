...
// Replace upload-cta-frame
html = html.replace(/<div class="upload-cta-frame">[\s\S]*?<\/div>\s*<\/div>\s*<!-- Dynamic High-Tech Progress Modal/m, newHeroForm + '\n  </div>\n\n  <!-- Dynamic High-Tech Progress Modal');

// 2. JavaScript 로직 전면 개편
const newFreeFunnelScript = `
    // ══════════════════════════════════════════════════════════════
    ...
    // Initialize OTP input interactions
    ...
    function setupOtpInputHandlers() {
      ...
      inputs.forEach((input, index) => {
        input.addEventListener('input', (e) => {
          ...
          // Check if all 6 digits entered -> Auto verify
    ...
    function handleFileSelected(input) {
      if (input.files && input.files[0]) {
        ...
        const title = document.getElementById('drop-title-txt');
        const hint = document.getElementById('drop-filename-txt');
        if (title) title.textContent = 'Photo Selected';
        if (hint) hint.textContent = selectedFile.name + ' (' + Math.round(selectedFile.size / 1024) + ' KB)';
        hideFormInlineError();
      }
    }
    ...
    async function handleVerifyOtpClick() {
      ...
      try {
        ...
        // Proceed directly to booth generation
...
// Replace existing free funnel script in index.html
const oldScriptStart = html.indexOf('let isDeveloperBypass = false;');
const oldScriptEnd = html.indexOf('function openPlanModal(planType)');

if (oldScriptStart !== -1 && oldScriptEnd !== -1) {
  html = html.substring(0, oldScriptStart) + newFreeFunnelScript.trim() + '\n\n    ' + html.substring(oldScriptEnd);
  console.log('✅ index.html free-funnel scripts cleanly replaced');
} else {
  console.log('⚠️ Could not locate exact script boundaries, searching...');
}
