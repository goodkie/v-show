...
// 1. cb1df55 커밋에서 완전한 원본 index.html 가져오기
const pristineHtml = execSync('git show cb1df55:virtual-tradeshow-commercial-v1/app_build/client/index.html', { maxBuffer: 10 * 1024 * 1024 }).toString('utf8');
...
html = html.replace(/<a href="\/" class="brand-logo"[\s\S]*?<\/a>/, brandLogoLanding);

// (B) Hero Pill 문구: YOUR BOOTH, BEYOND THE SHOW.
html = html.replace('ONE PHOTO • PHOTO IMMERSIVE BOOTH', 'YOUR BOOTH, BEYOND THE SHOW.');

// (C) How It Works STEP 03: Setup Product Pins
html = html.replace('Setup 3 Product Pins', 'Setup Product Pins');

// (D) 상단 네비게이션 메뉴 (파트너쉽 없이 핵심 메뉴만 유지)
const navLinksClean = `<div class="nav-links">
...
html = html.replace(/<div class="nav-links">[\s\S]*?<\/div>/, navLinksClean);

// (E) 푸터 사이트맵에 Partnerships & Affiliates 추가
...
html = html.replace(/<footer class="footer">[\s\S]*?<\/footer>/, footerSitemap);

// (F) 비디오 autoplay loop muted playsinline preload="auto" 설정
html = html.replace(
  `src="/assets/demo/virtual-fitting-room/fashion.mp4" playsinline muted loop preload="metadata"`,
  `src="/assets/demo/virtual-fitting-room/fashion.mp4" autoplay loop muted playsinline preload="auto"`
);
html = html.replace(
  `src="/assets/demo/virtual-makeup-artist/makeup.mp4" playsinline muted loop preload="metadata"`,
  `src="/assets/demo/virtual-makeup-artist/makeup.mp4" autoplay loop muted playsinline preload="auto"`
);

// (G) 100% 영문 Strategic Partnerships & Affiliates 모달 주입
const modalEnglish = `
  <!-- Partnerships & Consultation Modal (100% English) -->
...
// 기존 consultation-modal을 새 영문 모달로 깔끔하게 교체
if (html.includes('id="consultation-modal"')) {
  // 모달 태그의 시작부터 끝 </div>까지 교체
  const startIdx = html.indexOf('<div id="consultation-modal"');
  const endIdx = html.indexOf('</body>');
  html = html.substring(0, startIdx) + modalEnglish.trim() + '\n\n' + html.substring(endIdx);
} else {
  html = html.replace('</body>', `${modalEnglish.trim()}\n</body>`);
}
  ...
  <script>
    ...
    async function handleConsultationSubmit(e) {
      ...
      try {
        ...
        if (refIdEl) {
          refIdEl.textContent = '3DNA-PTN-' + Math.random().toString(36).substring(2, 8).toUpperCase();
        }
      } catch (err) {
        if (errEl) {
...
// </head> 바로 전에 모달 스크립트 안전하게 주입
html = html.replace('</head>', `${modalScript}\n</head>`);

// 저장
fs.writeFileSync(path.join(baseDir, 'app_build', 'client', 'index.html'), html, 'utf8');
...
console.log('✅ Pristine index.html reconstructed and patched with 100% integrity (Total lines: ' + html.split('\n').length + ')');
