...
const artifactDir = path.resolve('E:/vivpr/ai/v-show/production_artifacts/3dna_c11_11_p0_3dz');
if (!fs.existsSync(artifactDir)) {
  fs.mkdirSync(artifactDir, { recursive: true });
}

// 1. Audit Report
const auditReport = `# 3DZ — FREE 3D BOOTH FUNNEL COMPLETE FORENSIC AUDIT & REPAIR REPORT
...
fs.writeFileSync(path.join(artifactDir, 'AUDIT_AND_REPAIR_REPORT.md'), auditReport, 'utf8');

// 2. Update 3dna_brain_state.json
...
brainState.brand = '3DZ';
brainState.productName = '3DZ Spatial Virtual Showrooms & 3D Booths';
brainState.customerFacingBrand = '3DZ';
brainState.customerFacingDomain = '3dz.site';
brainState.emailSender = '3DZ 3D Booth <verify@mail.3dz.site>';
brainState.lastVerifiedMilestone = '3DZ-P0-FUNNEL-VERIFIED';
brainState.verifiedAt = new Date().toISOString();
brainState.free3dBoothFunnel = {
  status: 'REPAIRED_AND_VERIFIED',
  stateMachine: '10_STATES_DETERMINISTIC',
  dataReentry: 0,
  r2Tier0BackupActive: true,
  productSlotsIncluded: 3,
  emailProvider: 'RESEND',
  senderDomain: 'mail.3dz.site',
  humanInboxReceiptGate: 'OWNER_EXTERNAL_MAILBOX_RECEIPT_REQUIRED'
};

fs.writeFileSync(brainStatePath, JSON.stringify(brainState, null, 2), 'utf8');
console.log('✅ Generated production artifacts and updated 3dna_brain_state.json');