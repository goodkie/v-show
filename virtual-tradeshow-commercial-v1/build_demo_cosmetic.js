...
let cosmeticHtml = baseHtml
  ...
  .replace(/<div class="brand-group">.*?<\/div>/s, `
    <div class="brand-group">
      <a href="/" class="brand-logo">
        <span style="font-family:'Plus Jakarta Sans', sans-serif; font-size: 19px; letter-spacing: 1.5px; font-weight: 900; color: #fff;">LUMIÈRE</span>
        <span style="font-size: 10px; letter-spacing: 2px; color: #e2c974; font-weight: 700; margin-left: 4px;">SKINCARE</span>
      </a>
      <span class="brand-badge" style="background: rgba(226, 201, 116, 0.12); border-color: rgba(226, 201, 116, 0.35); color: #f3e8c8;">
        <span class="pulse-dot" style="background: #e2c974; box-shadow: 0 0 8px #e2c974;"></span>
        CLEAN BEAUTY &amp; BIO-TECH PAVILION
      </span>
    </div>
  `)
  .replace(/DOWNLOAD BROCHURE/g, 'DOWNLOAD CATALOG')
...
// Update CSS Variables for Luxury Gold & Botanical Theme
cosmeticHtml = cosmeticHtml
  .replace(/--cyan: #00c2ff;/g, '--cyan: #e2c974;')
  .replace(/--cyan-glow: rgba\(0, 194, 255, 0.55\);/g, '--cyan-glow: rgba(226, 201, 116, 0.55);')
  .replace(/--panel-border: rgba\(56, 189, 248, 0.35\);/g, '--panel-border: rgba(226, 201, 116, 0.35);');

// Make product titles white in CSS
cosmeticHtml = cosmeticHtml
  .replace(/\.prod-card-cat \{ font-size: 8\.5px; font-weight: 800; color: var\(--cyan\);/g, '.prod-card-cat { font-size: 8.5px; font-weight: 800; color: #ffffff;')
  .replace(/\.hotspot-cat \{ font-size: 6\.5px; font-weight: 800; color: var\(--cyan\);/g, '.hotspot-cat { font-size: 6.5px; font-weight: 800; color: #ffffff; opacity: 0.85;')
  .replace(/\.hotspot-label-text \{ font-size: 10px; font-weight: 800; color: #fff;/g, '.hotspot-label-text { font-size: 10px; font-weight: 800; color: #ffffff;');
...
const PRODUCTS_DATA = [
  {
    ...
    worldPos: new THREE.Vector3(-35, -65, -350),
    nodeIdx: 0,
  ...
  },
  {
    ...
    worldPos: new THREE.Vector3(-180, -75, -320),
    nodeIdx: 0,
  ...
  },
  {
    ...
    worldPos: new THREE.Vector3(45, -65, -350),
    nodeIdx: 0,
  ...
  },
  {
    ...
    worldPos: new THREE.Vector3(175, -20, -330),
    nodeIdx: 0,
...
cosmeticHtml = cosmeticHtml
  ...
  .replace(/<div class="spec-list" id="side-spec-list">.*?<\/div>/s, `
    <div class="spec-list" id="side-spec-list">
      <div class="spec-item"><div class="spec-k">ACTIVE</div><div class="spec-v">15% Vitamin C</div></div>
      <div class="spec-item"><div class="spec-k">FORMULA</div><div class="spec-v">98% Natural Bio-Tech</div></div>
  ...
  `)
  .replace(/📝 Request 1:1 Technical & RFQ Quote/g, '📝 Request Wholesale / OEM Quote')
  .replace(/📝 Request Wholesale Pricing/g, '📝 Request Wholesale / OEM Quote')
  ...
  .replace(/01\. Main Booth Center/g, '01. Lumière Main Exhibition Pavilion');
