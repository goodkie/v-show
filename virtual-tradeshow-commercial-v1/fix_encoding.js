...
<title>DN'a ROBOTIC | Matterport 3D Digital Twin Virtual Tour</title>
...
#viewer-container { position: absolute; inset: 0; width: 100%; height: 100%; overflow: hidden; }
...
.mattertag-element { position: absolute; transform: translate(-50%, -100%); pointer-events: auto; cursor: pointer; }
.mattertag-beacon {
  width: 22px; height: 22px; border-radius: 50%; margin: 0 auto 4px;
  background: var(--cyan); box-shadow: 0 0 0 4px rgba(0,194,255,0.25), 0 0 20px rgba(0,194,255,0.5);
  animation: beacon-pulse 2.2s infinite;
}
@keyframes beacon-pulse { 0%,100%{box-shadow:0 0 0 4px rgba(0,194,255,0.25),0 0 20px rgba(0,194,255,0.5);}50%{box-shadow:0 0 0 10px rgba(0,194,255,0.08),0 0 30px rgba(0,194,255,0.7);} }
.mattertag-card {
  background: var(--panel-bg); border: 1px solid var(--panel-border);
...
<div id="topbar">
  <div class="brand-group">
    ...
    <div class="brand-badge"><div class="pulse-dot"></div> MATTERPORT 3D DIGITAL TWIN</div>
  </div>
...
<!-- SPATIAL NODES PANEL -->
<div id="node-panel">
  <div class="panel-label">SPATIAL NODES</div>
  <button class="node-btn active" id="nb-0" onclick="switchNode(0)"><span class="node-icon">📷</span> 01. 부스 메인 조망 (Entrance)</button>
  <button class="node-btn" id="nb-1" onclick="switchNode(1)"><span class="node-icon">🤖</span> 02. 전면 코봇 라인업 (Robots)</button>
  <button class="node-btn" id="nb-2" onclick="switchNode(2)"><span class="node-icon">⚙</span> 03. 메인 로봇 스테이션 (Center)</button>
  <button class="node-btn" id="nb-3" onclick="switchNode(3)"><span class="node-icon">🚗</span> 04. AMR &amp; 센서 존 (Runway)</button>
  <button class="node-btn" id="nb-4" onclick="switchNode(4)"><span class="node-icon">🖥</span> 05. AI 비전 미디어 월 (LED Wall)</button>
  <button class="node-btn" id="nb-5" onclick="switchNode(5)"><span class="node-icon">🛋</span> 06. VIP 라운지 &amp; 상담 (Lounge)</button>
</div>
...
<div id="booth-radar">
  <div class="radar-label">
    ...
    <span id="radar-loc-txt">01. ENTRANCE</span>
  </div>
...
<div id="product-modal" onclick="if(event.target===this)closeModal()">
  <div class="modal-box">
...
<script>
/* ═══════════════════════════════════════════════════════════
   MATTERPORT 3D SPATIAL DIGITAL TWIN ENGINE  v3.1
   Full DPR · Anisotropy · 128x64 Sphere · Anti-Fisheye
   Spherical-Coordinate Mattertag Pins · UTF-8 Clean
═══════════════════════════════════════════════════════════ */

// 1. SPATIAL NODES
const SPATIAL_NODES = [
  { id:0, name:"01. 부스 메인 조망 (Entrance)",     image:"/assets/demo/dna-showcase/hero/dna_robotic_master_ultrahd.jpg",    camLook:{x:0,y:1.5,z:80},  radarPos:{x:73,y:100} },
  { id:1, name:"02. 전면 코봇 라인업 (Robots)",     image:"/assets/demo/dna-showcase/angles/angle_eye_01.jpg",               camLook:{x:-40,y:1.2,z:60},radarPos:{x:42,y:72} },
  { id:2, name:"03. 메인 로봇 스테이션 (Center)",   image:"/assets/demo/dna-showcase/angles/angle_6ft_01.jpg",               camLook:{x:30,y:1.2,z:60}, radarPos:{x:95,y:72} },
  { id:3, name:"04. AMR & 센서 존 (Runway)",        image:"/assets/demo/dna-showcase/angles/angle_floor_01.jpg",             camLook:{x:0,y:0.5,z:80},  radarPos:{x:55,y:48} },
  { id:4, name:"05. AI 비전 미디어 월 (LED Wall)",  image:"/assets/demo/dna-showcase/hero/dna_robotic_master_ultrahd.jpg",   camLook:{x:-15,y:2.0,z:80},radarPos:{x:73,y:28} },
  { id:5, name:"06. VIP 라운지 & 상담 (Lounge)",    image:"/assets/demo/dna-showcase/hero/dna_robotic_master_ultrahd.jpg",   camLook:{x:-30,y:1.2,z:70},radarPos:{x:30,y:20} }
];

// 2. SPHERICAL PIN HELPER
// az = horizontal angle (deg) from forward axis: + = right, - = left
// el = vertical angle (deg) from top pole: 90 = horizon, >90 = below
function sphPin(azDeg, elDeg) {
  const R = 70;
  const az = azDeg * Math.PI / 180;
  const el = elDeg * Math.PI / 180;
  return new THREE.Vector3(R * Math.sin(el) * Math.sin(az), R * Math.cos(el), R * Math.sin(el) * Math.cos(az));
}

// 3. MATTERTAGS — Spherical coords mapped to actual photo positions
const MATTERTAGS = [
  {
    id:'cobot-lineup', badge:'Collaborative Robot Array',
    title:"DN'a CoBot X16 — 7대 라인업",
    desc:'7대 연동 6축 협동로봇 조립 라인 — ±0.025mm 반복 정밀도, AI 비전 통합',
    worldPos: sphPin(-42, 90), // 왼쪽 코봇 진열대
    specs:[['가반하중','16.0 kg'],['작업반경','1300 mm'],['반복정밀도','±0.025 mm'],['안전등급','ISO TS 15066'],['통신규격','EtherCAT / PROFINET'],['가격대','$38,500 – $42,000']]
  },
  {
    id:'center-cobot', badge:'Featured Workstation',
    title:"DN'a Active Dual-Grip Cell",
    desc:'서보 2핑거 적응형 그리퍼 장착 고속 머신 텐딩 스테이션',
    worldPos: sphPin(36, 88), // 오른쪽 코봇 스테이션
    specs:[['그립력','220 N'],['스트로크','80 mm'],['사이클 타임','0.8 초'],['적용분야','고속 픽앤플레이스'],['제어기','통합 AI 비전 PC'],['가격대','$45,000 – $52,000']]
  },
  {
    id:'media-wall', badge:'Digital Media Wall',
    title:"AI · Vision · Automation LED Wall",
    desc:'16:9 대형 미디어 파사드 — 실시간 스마트 팩토리 대시보드 및 로봇 제어 현황',
    worldPos: sphPin(-16, 74), // 상단 좌측 LED 스크린
    specs:[['해상도','4K UHD Fine-Pitch'],['휘도','1200 nits'],['화면크기','6000 x 3400 mm'],['인터페이스','Real-time Telemetry']]
  },
  {
    id:'amr-vector', badge:'Autonomous Logistics',
    title:"DN'a Vector AMR 600",
    desc:'Laser SLAM 자율주행 물류 로봇 — 최대 600kg 적재, 360° LiDAR 회피',
    worldPos: sphPin(4, 106), // 바닥 전면 AMR
    specs:[['적재중량','600 kg'],['최대속도','2.0 m/s'],['항법방식','3D LiDAR SLAM'],['연속작동','10 시간 (급속충전)'],['안전인증','ISO 3691-4'],['가격대','$34,000 – $39,500']]
  }
];
...
function initThree() {
  ...
  // Camera at origin — inside panorama sphere
  ...
  // Pure look-around controls — no zoom, no pan
  ...
  controls.maxPolarAngle = Math.PI * 0.70; // Prevent looking too far up/down (anti-fisheye)
  controls.minPolarAngle = Math.PI * 0.30;
  controls.rotateSpeed = -0.42;

  // High-quality 128x64 panorama sphere
  const sphereGeo = new THREE.SphereGeometry(100, 128, 64);
  sphereGeo.scale(-1, 1, 1);
  photoMaterial = new THREE.MeshBasicMaterial({ side: THREE.FrontSide, depthWrite: false });
...
// 5. FLOOR PUCKS (Matterport navigation rings — visible at bottom of panorama)
function buildFloorPucks() {
  SPATIAL_NODES.forEach((node, idx) => {
    const g = new THREE.Group();
    g.position.set((idx - 2.5) * 14, -24, 40);

    const ring = new THREE.Mesh(
      new THREE.RingGeometry(4.5, 6.5, 36),
...
function buildDollhouseMesh() {
  ...
  const floorMesh = new THREE.Mesh(new THREE.PlaneGeometry(16,16), new THREE.MeshBasicMaterial({ color:0x0284c7, transparent:true, opacity:0.18, side:THREE.DoubleSide }));
  floorMesh.rotation.x = -Math.PI / 2;
  dollhouseGroup.add(floorMesh);
  const grid = new THREE.GridHelper(16, 16, 0x00c2ff, 0x1e293b);
  grid.position.y = 0.01;
  dollhouseGroup.add(grid);
  dollhouseGroup.visible = false;
  scene.add(dollhouseGroup);
}
...
// 8. HIGH-QUALITY TEXTURE LOADING
...
function switchNode(idx) {
  ...
  if (currentMode === 'tour') {
    new TWEEN.Tween(controls.target)
      .to({ x: node.camLook.x, y: node.camLook.y, z: node.camLook.z }, 900)
      .easing(TWEEN.Easing.Cubic.Out).start();
  }

  floorPucks.forEach((p, i) => {
    p.children[0].material.opacity = (i === idx) ? 0.25 : 0.85;
    p.children[1].material.opacity = (i === idx) ? 0.35 : 0.9;
  });

  drawRadar();
  showToast('📍 ' + node.name + ' 실사 공간으로 이동했습니다.');
}
...
function setMode(mode) {
  ...
  const node = SPATIAL_NODES[currentNodeIdx];
  if (mode === 'tour') {
    dollhouseGroup.visible = false;
    ...
    showToast('📸 100% 실사 360° 파노라마 3D 공간 투어');
  } else if (mode === 'dollhouse') {
    dollhouseGroup.visible = true;
    photoSphere.visible = true;
    photoMaterial.transparent = true;
    photoMaterial.opacity = 0.15;
    new TWEEN.Tween(controls.target).to({x:0,y:-10,z:0}, 900).easing(TWEEN.Easing.Cubic.Out).start();
    showToast('🏠 3D Dollhouse 입체 투시도 모드');
  } else if (mode === 'floor') {
    dollhouseGroup.visible = true;
    photoSphere.visible = false;
    new TWEEN.Tween(controls.target).to({x:0,y:-50,z:1}, 900).easing(TWEEN.Easing.Cubic.Out).start();
...
function toggleAutoTour() {
  ...
  if (autoTourActive) {
    ...
    showToast('▶ 4.8초 간격 Matterport 자동 투어 시작');
    let ni = (currentNodeIdx + 1) % SPATIAL_NODES.length;
    switchNode(ni);
    autoTourTimer = setInterval(() => { ni = (currentNodeIdx + 1) % SPATIAL_NODES.length; switchNode(ni); }, 4800);
  }
}
...
function drawRadar() {
  ...
  const angle = Math.atan2(controls.target.x - camera.position.x, controls.target.z - camera.position.z);
...
// 13. MATTERTAG SCREEN PROJECTION
function updateMattertags() {
  MATTERTAGS.forEach(tag => {
    if (!tag.domElement) return;
    const wp = tag.worldPos.clone();
    wp.project(camera);
    if (wp.z > 1.0) { tag.domElement.style.display = 'none'; return; }
    tag.domElement.style.display = currentMode === 'tour' ? 'block' : 'none';
