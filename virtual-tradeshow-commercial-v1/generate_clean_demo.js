...
.btn-ui.matterport { background: rgba(0,194,255,0.15); border-color: var(--cyan); color: var(--cyan); }
.btn-ui.matterport:hover { background: var(--cyan); color: #000; box-shadow: 0 0 16px var(--cyan-glow); }

/* MAIN SHOWROOM WORKSPACE (3D Viewport + Right Unified Sidebar) */
#studio-workspace {
  flex: 1; display: grid;
...
/* ══════════════════════════════════════════════
   3D FLOATING PRODUCT HOTSPOTS (MATCHING 64K STUDIO)
══════════════════════════════════════════════ */
    ...
    <aside class="unified-sidebar">
      ...
      <div class="radar-box">
        <div class="radar-header">
          ...
          <span id="radar-loc-txt">01. MAIN BOOTH</span>
        </div>
...
<script>
/* ═══════════════════════════════════════════════════════════
   3D VIRTUAL SHOWROOM ENGINE (v8.0 Master)
   - 4 Identical Robotic Stations Aligned with Matterport Studio
   - Sophisticated Procedural Robot 3D Models & PBR Pedestals
   - Permanent 3D Floating Capsules with Live Mini-Thumbnails
   - Interactive 360 Turntable Mini Viewer inside Product Drawer
   - 100% Full English Localization
═══════════════════════════════════════════════════════════ */
...
const PRODUCTS_DATA = [
  {
    ...
    pos3d: new THREE.Vector3(0, 0, -2.2), // Center circular pedestal
    cameraPos: new THREE.Vector3(0, 2.2, 2.0),
    cameraLook: new THREE.Vector3(0, 1.2, -2.2),
    radarPos: { x: 153, y: 68 },
  ...
  },
  {
    ...
    pos3d: new THREE.Vector3(-4.6, 0, 1.8), // Left logistics dock
    cameraPos: new THREE.Vector3(-3.2, 1.8, 4.8),
    cameraLook: new THREE.Vector3(-4.6, 0.5, 1.8),
    radarPos: { x: 72, y: 48 },
  ...
  },
  {
    ...
    pos3d: new THREE.Vector3(4.6, 0, 1.8), // Right packaging station
    cameraPos: new THREE.Vector3(3.2, 1.8, 4.8),
    cameraLook: new THREE.Vector3(4.6, 1.2, 1.8),
    radarPos: { x: 234, y: 48 },
  ...
  },
  {
    ...
    pos3d: new THREE.Vector3(-3.2, 0, -4.8), // Upper precision cell
    cameraPos: new THREE.Vector3(-1.8, 1.8, -2.0),
    cameraLook: new THREE.Vector3(-3.2, 1.0, -4.8),
    radarPos: { x: 92, y: 82 },
...
function initShowroom() {
  ...
  controls.maxPolarAngle = Math.PI * 0.48; // Prevent going under floor
...
function buildBoothArchitecture() {
  ...
  scene.add(new THREE.AmbientLight(0xd0e2ff, 0.7));
  const keySun = new THREE.DirectionalLight(0xffffff, 1.4);
  keySun.position.set(5, 14, 8);
  keySun.castShadow = true;
  keySun.shadow.mapSize.set(1024, 1024);
  scene.add(keySun);

  const skyLight = new THREE.HemisphereLight(0x38bdf8, 0x050b16, 0.6);
  scene.add(skyLight);
  ...
  // Booth Carpet & Cyan Perimeter Glow
  const carpetMat = new THREE.MeshStandardMaterial({ color: 0x0b172a, roughness: 0.8, metalness: 0.2 });
  ...
  const ledWallGeo = new THREE.CylinderGeometry(14, 14, 5.2, 32, 1, true, -Math.PI * 0.35, Math.PI * 0.7);
  ...
  ledWall.position.set(0, 2.6, -7.5);
  scene.add(ledWall);
...
function buildProductStations() {
  ...
  const matMetal = new THREE.MeshStandardMaterial({ color: 0x334155, roughness: 0.35, metalness: 0.85 });
  const matCyan = new THREE.MeshStandardMaterial({ color: 0x00c2ff, emissive: 0x00c2ff, emissiveIntensity: 0.7, roughness: 0.1 });
  const matGold = new THREE.MeshStandardMaterial({ color: 0xf59e0b, roughness: 0.2, metalness: 0.8 });
  ...
  PRODUCTS_DATA.forEach((prod, idx) => {
    ...
    if (prod.robotType === 'cobot') {
      // ── Station 0: Center Circular Pedestal with Apex Cobot X16 ──
      const plinthBase = new THREE.Mesh(new THREE.CylinderGeometry(1.6, 1.8, 0.45, 36), matMetal);
      plinthBase.position.y = 0.225;
      plinthBase.receiveShadow = true;
      plinthBase.castShadow = true;
      stationGroup.add(plinthBase);

      const plinthRing = new THREE.Mesh(new THREE.TorusGeometry(1.62, 0.025, 12, 36), matCyan);
      plinthRing.rotation.x = Math.PI / 2;
      plinthRing.position.y = 0.44;
      stationGroup.add(plinthRing);

      // Cobot 6-Axis Arm
      const base = new THREE.Mesh(new THREE.CylinderGeometry(0.35, 0.42, 0.25, 24), matMetal);
      base.position.y = 0.57;
      stationGroup.add(base);

      const shoulder = new THREE.Mesh(new THREE.SphereGeometry(0.24, 20, 20), matCyan);
      shoulder.position.y = 0.8;
      stationGroup.add(shoulder);

      const upperArm = new THREE.Mesh(new THREE.CylinderGeometry(0.15, 0.17, 0.9, 20), matWhite);
      upperArm.position.set(0.18, 1.25, 0.1);
      upperArm.rotation.set(0.35, 0, 0.25);
      stationGroup.add(upperArm);

      const elbow = new THREE.Mesh(new THREE.SphereGeometry(0.2, 20, 20), matMetal);
      elbow.position.set(0.3, 1.68, 0.2);
      stationGroup.add(elbow);

      const forearm = new THREE.Mesh(new THREE.CylinderGeometry(0.12, 0.14, 0.75, 20), matWhite);
      forearm.position.set(0.12, 2.0, 0.05);
      forearm.rotation.set(-0.45, 0, -0.3);
      stationGroup.add(forearm);

      const wrist = new THREE.Mesh(new THREE.CylinderGeometry(0.1, 0.1, 0.2, 16), matCyan);
      wrist.position.set(-0.08, 2.3, -0.1);
      stationGroup.add(wrist);

      const gripperL = new THREE.Mesh(new THREE.BoxGeometry(0.04, 0.18, 0.04), matMetal);
      gripperL.position.set(-0.14, 2.45, -0.1);
      const gripperR = new THREE.Mesh(new THREE.BoxGeometry(0.04, 0.18, 0.04), matMetal);
      gripperR.position.set(-0.02, 2.45, -0.1);
      const workpiece = new THREE.Mesh(new THREE.CylinderGeometry(0.045, 0.045, 0.1, 16), matGold);
      workpiece.position.set(-0.08, 2.45, -0.1);
      stationGroup.add(gripperL, gripperR, workpiece);

    } else if (prod.robotType === 'amr') {
      // ── Station 1: Vector AMR 600 Logistics Dock ──
      const dockPad = new THREE.Mesh(new THREE.BoxGeometry(2.4, 0.06, 1.8), matMetal);
      dockPad.position.y = 0.03;
      stationGroup.add(dockPad);

      const chargeStrip = new THREE.Mesh(new THREE.BoxGeometry(2.44, 0.02, 0.1), matCyan);
      chargeStrip.position.set(0, 0.07, 0.85);
      stationGroup.add(chargeStrip);

      const amrBody = new THREE.Mesh(new THREE.BoxGeometry(1.6, 0.42, 1.1), matWhite);
      amrBody.position.y = 0.32;
      stationGroup.add(amrBody);

      const amrGlow = new THREE.Mesh(new THREE.BoxGeometry(1.64, 0.04, 1.14), matCyan);
      amrGlow.position.y = 0.3;
      stationGroup.add(amrGlow);

      const lidar = new THREE.Mesh(new THREE.CylinderGeometry(0.14, 0.14, 0.16, 20), matCyan);
      lidar.position.set(0.65, 0.6, 0.4);
      stationGroup.add(lidar);

      for (let i = -0.55; i <= 0.55; i += 0.22) {
        const roller = new THREE.Mesh(new THREE.CylinderGeometry(0.035, 0.035, 0.9, 12), matMetal);
        roller.rotation.z = Math.PI / 2;
        roller.position.set(0, 0.58, i);
        stationGroup.add(roller);
      }

    } else if (prod.robotType === 'delta') {
      // ── Station 2: Titan Delta D12 High-Speed Station ──
      const table = new THREE.Mesh(new THREE.BoxGeometry(2.2, 0.8, 1.6), matMetal);
      table.position.y = 0.4;
      stationGroup.add(table);

      const conveyor = new THREE.Mesh(new THREE.BoxGeometry(2.4, 0.1, 0.6), matWhite);
      conveyor.position.set(0, 0.85, 0);
      stationGroup.add(conveyor);

      // Delta Top Frame
      const topFrame = new THREE.Mesh(new THREE.CylinderGeometry(0.75, 0.75, 0.15, 6), matMetal);
      topFrame.position.y = 2.4;
      stationGroup.add(topFrame);

      const deltaPlate = new THREE.Mesh(new THREE.CylinderGeometry(0.28, 0.28, 0.08, 6), matCyan);
      deltaPlate.position.y = 1.35;
      stationGroup.add(deltaPlate);

      // 3 Carbon Struts
      for (let i = 0; i < 3; i++) {
        const ang = (i * Math.PI * 2) / 3;
        const strut = new THREE.Mesh(new THREE.CylinderGeometry(0.025, 0.025, 1.15, 8), matWhite);
        strut.position.set(Math.cos(ang) * 0.45, 1.85, Math.sin(ang) * 0.45);
        stationGroup.add(strut);
      }

      const suction = new THREE.Mesh(new THREE.ConeGeometry(0.14, 0.18, 16), matCyan);
      suction.rotation.x = Math.PI;
      suction.position.y = 1.2;
      stationGroup.add(suction);

    } else if (prod.robotType === 'scara') {
      // ── Station 3: Hyperion SCARA S8 Precision Workstation ──
      const bench = new THREE.Mesh(new THREE.BoxGeometry(2.0, 0.85, 1.4), matMetal);
      bench.position.y = 0.425;
      stationGroup.add(bench);

      const esdMat = new THREE.Mesh(new THREE.PlaneGeometry(1.8, 1.2), matCyan);
      esdMat.rotation.x = -Math.PI / 2;
      esdMat.position.set(0, 0.86, 0);
      stationGroup.add(esdMat);

      const scaraCol = new THREE.Mesh(new THREE.CylinderGeometry(0.28, 0.35, 0.9, 24), matWhite);
      scaraCol.position.set(-0.4, 1.3, 0);
      stationGroup.add(scaraCol);

      const arm1 = new THREE.Mesh(new THREE.BoxGeometry(0.65, 0.18, 0.26), matWhite);
      arm1.position.set(-0.1, 1.8, 0);
      stationGroup.add(arm1);

      const joint2 = new THREE.Mesh(new THREE.CylinderGeometry(0.18, 0.18, 0.24, 20), matCyan);
      joint2.position.set(0.2, 1.82, 0);
      stationGroup.add(joint2);

      const arm2 = new THREE.Mesh(new THREE.BoxGeometry(0.55, 0.15, 0.22), matWhite);
      arm2.position.set(0.45, 1.85, 0);
      stationGroup.add(arm2);

      const zShaft = new THREE.Mesh(new THREE.CylinderGeometry(0.04, 0.04, 0.7, 16), matMetal);
      zShaft.position.set(0.7, 1.7, 0);
      stationGroup.add(zShaft);

      const microWafer = new THREE.Mesh(new THREE.CylinderGeometry(0.1, 0.1, 0.02, 20), matGold);
      microWafer.position.set(0.7, 1.3, 0);
...
// 5. 3D FLOATING HOTSPOTS (PERMANENT TITLE & MINI THUMBNAIL)
