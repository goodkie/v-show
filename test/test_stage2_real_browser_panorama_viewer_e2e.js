...
async function main() {
  ...
  const testReceipt = {
    testMilestone: 'STAGE2-PANORAMA-BROWSER-OPTICAL-PROOF-R60',
    revisedPer: 'ChatGPT audit IC_kwDOT53X288AAAABWleCiw',
  ...
  try {
    ...
    testReceipt.gates = {
      ...
      SYNTHETIC_SIGNED_REAL_EXPRESS_ROUTE_TEST: 'AGENT_REPORTED_PASS (test_stage2_stripe_signed_route_e2e.js, 49/49 tests passed, authoritative stripe provider lookup, zero client network fault headers, provider-absent 503 fail-closed, incomplete pagination 502 fail-closed, repeated cursor detection, status completeness, metadata forgery rejection, owner grant routes & provenance, full commercial event provider authority, grant referential integrity, tamper-evident hash-chained grant audit trail, detached external audit root anchor, subscription period reconciliation, strict non-conditional identity)',
    ...
    const receiptPath = path.join(receiptsDir, 'R60_PANORAMA_BROWSER_OPTICAL_PROOF_RECEIPT.json');
    fs.writeFileSync(receiptPath, JSON.stringify(testReceipt, null, 2));
    console.log(`\n[RECEIPT] Saved R60 receipt to isolated scratch: scratch/test_receipts/R60_PANORAMA_BROWSER_OPTICAL_PROOF_RECEIPT.json`);
